...
const RoomPage = () => {
  ...
  useEffect(() => {
    ...
    bindMonacoToYjs(activePath, providerRef.current, editorRef.current);
  }, [activePath]);
  ...
  useEffect(() => {
    ...
    const onRoomJoined = (payload: {
      ...
      window.setTimeout(() => {
        bindMonacoToYjs(nextActive, providerRef.current, editorRef.current);
      }, 50);
    };
      ...
      <header className="h-12 bg-surface-container border-b border-outline-variant/30 flex items-center justify-between px-4 z-50 shrink-0">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2 font-bold text-primary tracking-tight">
            ...
            <span className="hidden sm:inline">CodeSync</span>
          </div>
          {/* Breadcrumbs Path */}
          ...
          <div className="p-4 border-t border-outline-variant/20 flex flex-col gap-2">
            ...
            <div className="flex flex-col xl:flex-row gap-2 pb-2">
              <input
                ...
                className="flex-1 min-w-0 shrink bg-surface-container border border-outline-variant/30 rounded px-2 py-1.5 text-xs text-on-surface focus:outline-none focus:border-primary" placeholder="Rename to..." />
              <button disabled={(!activePath && !selectedFolderPath)} onClick={selectedFolderPath ? handleRenameFolder : handleRenameFile} className="px-3 py-1.5 shrink-0 bg-surface-variant hover:bg-surface-bright disabled:opacity-50 transition-colors rounded text-xs font-semibold cursor-pointer">Rename</button>
            </div>
