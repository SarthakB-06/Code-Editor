...
const buildInitialFs = async (
  ...
  if (
    existingDoc &&
    typeof existingDoc.code === "string" &&
    existingDoc.code.length > 0
  ) {
    const folders = getParentFoldersForPath(DEFAULT_FILE_PATH);
    return {
      version:
        typeof existingDoc.version === "number" ? existingDoc.version : 0,
      folders,
...
export const getOrCreateRoomFs = async (roomId: string): Promise<RoomFs> => {
  ...
  if (existing) {
    ...
    for (const f of files)
      derivedFolders.push(...getParentFoldersForPath(f.path));

    const folders = dedupeFolderPaths([
      ...(fs.folders ?? []),
      ...derivedFolders,
    ]);
